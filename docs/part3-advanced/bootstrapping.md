...
$$\text{ct} = (\mathbf{a},\ b), \quad b = \mathbf{a} \cdot \mathbf{s} + m \cdot \lfloor q/t \rfloor + e$$
...
$$b - \mathbf{a} \cdot \mathbf{s} = m \cdot \lfloor q/t \rfloor + e \xrightarrow{\text{取整}} m$$

只要 $e$ 足够小，取整能正确恢复 $m$。一旦 $e$ 超过阈值 $q/2t$，解密就会出错。

### 乘法让噪声爆炸

两个密文相乘时，噪声项展开大约是：

$$e_{\text{new}} \approx m_1 \cdot e_2 + m_2 \cdot e_1 + e_1 \cdot e_2$$
...
解密函数本身只是一个关于密文 $(\mathbf{a}, b)$ 和私钥 $\mathbf{s}$ 的函数：

$$\text{Dec}(\mathbf{s},\ \text{ct}) = \left\lfloor \frac{t}{q} \cdot (b - \mathbf{a} \cdot \mathbf{s}) \right\rceil \mod t$$
...
把私钥 $\mathbf{s}$ 的每一位，用一把新的私钥 $\mathbf{s}'$ 重新加密：

$$\text{bk} = \{ \text{Enc}_{\mathbf{s}'}(s_0),\ \text{Enc}_{\mathbf{s}'}(s_1),\ \ldots,\ \text{Enc}_{\mathbf{s}'}(s_{n-1}) \}$$

这是一次性的初始化操作，bk 之后**永久保存，反复使用，不会被消耗**。

**第二步：把旧密文当"常数"**

原来噪声快满的 $\text{ct}_{\text{old}} = (\mathbf{a}, b)$，在 Bootstrapping 里不再是"密文"，而是**公开的常数输入**——就像普通的数字一样传进去。

**第三步：同态执行解密电路**

用 bk 里的加密私钥，同态地计算 $b - \mathbf{a} \cdot \mathbf{s}$，再取整。

全程 $\mathbf{s}$ 都处于加密状态，没有人看到私钥明文。

**第四步：输出新鲜密文**

$$\text{ct}_{\text{new}} = \text{Enc}_{\mathbf{s}'}(m), \quad e_{\text{new}} \ll e_{\text{old}}$$

### 旧噪声去哪了？

这是最关键的问题。

$\text{ct}_{\text{old}}$ 里的大噪声 $e_{\text{old}}$，在同态解密时，被取整操作**消化掉了**：

$$\left\lfloor \frac{t}{q}(m \cdot \lfloor q/t \rfloor + e_{\text{old}}) \right\rceil = m$$

$e_{\text{old}}$ 被当作误差舍入掉，**不传递到输出密文里**。

输出密文 $\text{ct}_{\text{new}}$ 的噪声，来自：
- bk 里各密文的初始噪声（新鲜的，很小）
- 执行解密电路时同态运算累积的噪声
...
$\text{ct}_{\text{new}}$ 就是一个普通的新鲜密文，数学结构和刚加密时完全一样，只不过用的是私钥 $\mathbf{s}'$ 而不是 $\mathbf{s}$。乘法深度预算完全恢复，可以继续做下一轮计算。
...
你用 $\mathbf{s}'$ 加密了 $\mathbf{s}$，如果 $\mathbf{s}' = \mathbf{s}$（私钥加密自己），这叫 **Key-Dependent Message（KDM）安全性**，也叫**循环安全（Circular Security）**。
...
在多项式环 $\mathbb{Z}_q[X]/(X^N+1)$ 里，定义一个**测试向量** $T(X)$，把系数排列成你想求值的函数的查找表：

$$T(X) = \sum_{i=0}^{N-1} t_i \cdot X^i$$

核心操作是同态地计算：

$$T(X) \cdot X^{b - \sum_i a_i \cdot s_i}$$

旋转之后，提取某个系数，就得到了函数在 $v = b - \sum a_i s_i$ 处的值——而且是全新的加密，噪声很小。
